...
THE STATE BEING FIXED. `ios/` was neither committed nor ignored. Exactly ONE file under it was
tracked - `ios/PearTune.xcodeproj/project.pbxproj` - alongside eighteen untracked ones, and the
root `.gitignore` had no `ios` entry at all. So the directory was one accidental `git add` from
some earlier session, and everything else about it was invisible.

WHY THAT MATTERED. `ios/PearTune/Info.plist` is the file whose LITERAL `CFBundleShortVersionString`
and `CFBundleVersion` decide what App Store Connect receives, and it was on the untracked side.
The 2026-08-17 rejected upload is the direct consequence: app.json and project.pbxproj both read
1.0.2 build 5 on both machines, the archive/sign/export all succeeded, and Apple got an IPA
stamped 1.0.1 build 3. Nobody thought to ask which file the IPA reads, because every comment in
the tree - `scripts/release.sh` step 0, `scripts/ios-appstore.sh`, `test/ios-version.test.js` -
said `ios/` was generated output and not committed. Those comments were 6/7 true. A comment that
is mostly true about a directory is worse than no comment, because it stops the question being
asked.

THE DECISION: commit `ios/` in full, the same way `android/` already is. Nineteen files, 228 KB,
no secrets (the entitlements file is an empty dict, there is no DEVELOPMENT_TEAM or provisioning
profile in the pbxproj, and `.xcode.env` says in its own header that it is meant to be versioned).
The alternative considered and rejected was gitignoring `ios/` entirely and running
`expo prebuild -p ios --clean` on every release so the tree could never be stale. That is purer,
but it moves an Xcode-project regeneration onto the release-day critical path, and release day is
the worst time to discover that a newer Expo generates a project the Mac cannot archive.

THE COST, AND THE GUARD THAT PAYS IT. Committed generated output drifts. `ios/` is regenerated by
`expo prebuild` from `app.json`, and the release script prebuilds only when `ios/` is MISSING -
which it now never is. So editing `app.json`'s `ios` block does not reach an archive on its own.
`test/ios-version.test.js` is the gate: it no longer skips, and on top of the version literals it
now asserts the bundle identifier and EVERY key declared under `expo.ios.infoPlist` actually
reached the committed plist. Proved it bites rather than merely passes: adding
`NSLocalNetworkUsageDescription` to `app.json` without prebuilding turns the suite red, and
removing it turns it green again.

Also corrected in the same change: the stale "ios/ is NOT committed" comments in
`scripts/release.sh` (two) and `scripts/ios-appstore.sh` (one), the root `.gitignore` (which now
says out loud that the absence of an `ios` entry is deliberate, and ignores the two CocoaPods
outputs that only ever exist on the Mac), and this repo's `CLAUDE.md`.

NOT changed, and logged in TODO.md instead: `metadata/ios/` - the App Store listing copy - is
untracked in the same accidental way. It is the same shape of problem (text Apple receives,
invisible to review) but it is listing metadata rather than the native project, so it gets its
own decision.
