...
mapfile -t FILES < <(cd "$STAGE" && find host client protocol -type f 2>/dev/null | sort)
[ "${#FILES[@]}" -gt 0 ] || { echo "nothing staged under $STAGE/{host,client,protocol}" >&2; exit 1; }

BACKUP="/home/umbrel/peartune-dev-backup-$(date +%s)"
echo "== backing up to $BACKUP =="
for f in "${FILES[@]}"; do
  mkdir -p "$BACKUP/$(dirname "$f")"
  docker cp "$CONTAINER:/app/$f" "$BACKUP/$f" 2>/dev/null || echo "   (new file) $f"
done
...
for f in "${FILES[@]}"; do
  ...
  b=$(docker exec "$CONTAINER" md5sum "/app/$f" | cut -d' ' -f1)
