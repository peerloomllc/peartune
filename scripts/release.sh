...
#
# ios/ is committed in this repo, the same way android/ is - generated by
# `expo prebuild`, and the generated tree lives in git (DECISIONS.md 2026-08-18).
# So this file is normally present and the sed normally runs. The existence check
# stays because a tree can legitimately lack it - a checkout where ios/ was wiped
# to force a clean regenerate - and an unconditional sed would abort the release
# under `set -e`. When it IS absent, step 2 prebuilds it and the generator reads
# app.json directly, so the values still land.
#
# Until 2026-08-18 this comment said ios/ was NOT committed. It was wrong in
...
if [ -f "$XCODE_PROJECT" ]; then
  ...
  echo "    $XCODE_PROJECT absent (ios/ is committed, so this tree is missing it) - step 2 will prebuild it."
fi
