...
# ios/ IS committed (DECISIONS.md 2026-08-18) but the .xcworkspace inside it is NOT -
# CocoaPods writes it, so it only exists once `pod install` has run on THIS machine.
# If the rsync brought no project, or pod install has not run, there is nothing to
# archive, and saying so beats a wall of xcodebuild output.
if [ ! -f "$REPO_ROOT/${XCODE_WORKSPACE}" ] && [ ! -d "$REPO_ROOT/${XCODE_WORKSPACE}" ]; then
  echo "Error: $REPO_ROOT/${XCODE_WORKSPACE} not found." >&2
  echo "  ios/ is committed, but ${XCODE_WORKSPACE} is generated by \`pod install\` on this" >&2
  echo "  machine. Check the rsync brought ios/ over, then run pod install in ios/." >&2
  exit 1
fi
